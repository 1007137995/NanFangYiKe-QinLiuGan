...
public class rotate : MonoBehaviour {  
	...
	void Update()    
	{
		...
		{    
            ...
            Vector3 Xoffset = new Vector3(0f, offset.x, 0f);//过滤掉鼠标在Y轴方向上的偏移量，只保留X轴的  
            transform.Rotate(Vector3.Cross(Xoffset, Vector3.right).normalized, offset.magnitude, Space.Self);  //旋转  
		...
		{    
            ...
            //			transform.Rotate(Vector3.Cross(XfinalOffset, Vector3.forward).normalized, angle * 2 * Time.deltaTime, Space.Self);    
